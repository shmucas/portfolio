...
export default function Statement() {
  return (
    <section className="pb-16">
      <h1 className="text-3xl sm:text-4xl font-bold text-neutral-50 tracking-tight leading-[1.12] mb-4 max-w-3xl">
      ...
      <div className="flex flex-wrap gap-x-10 gap-y-4 mt-8">
        {stats.map((s) => (
          <div key={s.label}>
            <div className="font-mono text-xl font-bold text-amber-400">{s.value}</div>
            <div className="text-[11px] text-neutral-500 leading-tight mt-0.5">{s.label}</div>
          </div>
        ))}
      </div>
    </section>
